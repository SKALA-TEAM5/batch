...
from src.refresh.diff_engine import (
...
def run_refresh(
    *,
    ...
    for diff in tqdm(diffs, desc="소스별 refresh 적용", unit="소스"):
...
def _apply_rdb_changes(database_url: str, run_id: str, diff: SourceDiff) -> None:
    ...
    with psycopg.connect(database_url) as conn:
        with conn.cursor() as cur:
            for doc in tqdm(diff.added, desc="추가", unit="건", leave=False):
            ...
            for doc in tqdm(diff.updated, desc="수정", unit="건", leave=False):
                _upsert_legal_master(cur, doc)
                insert_law_log(
                    cur,
                    build_log_row(
            ...
            for doc in tqdm(diff.deleted, desc="삭제", unit="건", leave=False):
                insert_law_log(
                    cur,
                    build_log_row(
                        run_id=run_id,
